...
if config["basecalling_mode"]=="dorado":
  ...
  rule basecall_guppy:
    ...
    threads: 4
    benchmark: "benchmarks/{sample}.{file}.basecall_guppy_cpu.benchmark.txt"
    shell:
      "mkdir {output.tmp} ; cp {input.fast5} {output.tmp} ; mkdir -p {output.out} ; "
  ...
  rule basecall_guppy:
    ...
    threads: 1
    resources: gpu=1
    benchmark: "benchmarks/{sample}.{file}.basecall_guppy_gpu.benchmark.txt"
    shell:
      "mkdir {output.tmp} ; cp {input.fast5} {output.tmp} ; mkdir -p {output.out} ; "
...
if config["basecalling_mode"]=="none_fastq":
  ...
  rule merge_fastq:
    ...
    output: "fastq/{sample}.fq"
    conda: "envs/minimap.yaml"
    benchmark: "benchmarks/{sample}.merge_fastq.benchmark.txt"
    shell: "find basecalling_parallel/{wildcards.sample}/ -name '*.bam' | xargs -n 1 samtools fastq -T MM,ML > {output}"
...
rule demux_QC:
    ...
    benchmark: "benchmarks/{sample}.demux_QC.benchmark.txt"
    shell:
