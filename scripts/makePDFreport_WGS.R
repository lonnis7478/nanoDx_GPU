...
if (snakemake@config[["cuda"]] != "enabled") {
...
if (snakemake@config[["cuda"]] != "disabled") {
    cudaTSNE <- normalizePath(snakemake@input[["cuda_tSNE"]])
    load(normalizePath(snakemake@input[["CUDAvotes"]]))
    row.names(votes) <- votes$Dx
    load(normalizePath(snakemake@input[["cuda_model_info"]]))
    if(snakemake@config[["cuda"]] == "compare"){
        ...
        render("scripts/WGSreportCUDAFast.Rmd",
               output_format = "pdf_document",
               output_dir=dirname(normalizePath(snakemake@output[[1]])),
               output_file=basename(normalizePath(snakemake@output[[1]]))
               )
