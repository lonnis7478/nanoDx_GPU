...
__device__ const int LEAF_NODES_PER_TREE = 128;
 ...
 */
__device__ void
reinitSharedValues(int *sampleSize, int maxFeature, int *bestFeature, bool *isLeaf, int *classDiscovered, int *classCount,
    ...
    for (int i = threadIdx.x; i < MAX_CLASS * maxFeature; i += blockDim.x) {
 ...
 */
__device__ void selectBestFeature(int maxFeature, float *bestGain, int *bestFeature, float *informationGains) {
...
__global__ void predictForest(const int *forest, const int *leafSamples, const int *d_x, const int *sample, int* validationFolds, int treeSize, int *predictions,
    ...
    if (tid < MAX_CLASS) {
        ...
        validationSample = 0;
        while(validationSample < VALIDATION_FOLD_SIZE) {
            (validationProba + (fold*VALIDATION_FOLD_SIZE + validationSample) * MAX_CLASS)[tid] /= (float) TREE_NUM;
            validationSample++;
        }
...
buildForest(int *X, int *y, int *mask, int *branchClassCounts, int *leafSamples, int *forest, int samples,
            int featureSize, int maxFeature, int seed, int validationFoldIndex, bool crossValidate) {
    ...
    int* sharedBranchSums = sharedData + SAMPLES * 3 + MAX_CLASS + maxFeature;
    float* sharedRightBranchEntropy = (float *) sharedData + SAMPLES * 3 + MAX_CLASS + maxFeature*2;
    float* sharedLeftBranchEntropy = (float *) sharedData + SAMPLES * 3 + MAX_CLASS + maxFeature*3;
    ...
    while (currentNode < maxNode) {
        ...
        reinitSharedValues(&sampleSize, maxFeature, &bestFeature, &isLeaf, &classDiscovered, classCount,
                           branchClassCounts + MAX_CLASS * maxFeature * blockIdx.x, tid);
        ...
        if (forest[maxNode * blockIdx.x + currentNode] != -1) {
            ...
            while (i < SAMPLES) {
                ...
                if (nodeMapping[maskIndex] == currentNode) {
                    ...
                    if (X[maskIndex * FEATURE_SIZE + features[threadIdx.y]] == 0) {
                        atomicAdd(&(branchClassCounts + blockIdx.x * MAX_CLASS * maxFeature +
            ...
            if (isLeaf || sampleSize <= 0 || sampleSize < MIN_SAMPLES_SPLIT || leftChild + 1 >= maxNode - 1 ||
                ...
                calculateEntropy(sampleSize, sharedBranchSums[threadIdx.y], tid, classCount,
                                 branchClassCounts + MAX_CLASS * maxFeature * blockIdx.x,
                ...
                selectBestFeature(maxFeature, &bestGain, &bestFeature, sharedLeftBranchEntropy);
 ...
 * @param featureSize
 */
void CudaCalibratedClassifierCV::fitBaseRF(int **d_tree, int **d_leafSamples, int **d_predictions, float **d_predictionProba, float **d_validationProba, int **d_branchClassCounts, int **d_mask, int **d_x, int **d_y, int **d_sample, int **h_predictions, int seed, int treeSize, int maxLeafNodes, int memory, int featureSize) {
    // Reset values
    cudaMemset(*d_tree, 0, sizeof(int) * treeSize * this->forestSize);
    cudaMemset(*d_leafSamples, 0, sizeof(int) * MAX_CLASS * maxLeafNodes * this->forestSize);
    cudaMemset(*d_predictions, 0, sizeof(int) * MAX_CLASS);
    cudaMemset(*d_predictionProba, 0, sizeof(float) * MAX_CLASS);
    cudaMemset(*d_validationProba, 0, sizeof(float) * MAX_CLASS * SAMPLES);

    dim3 buildGrid(this->forestSize, 1);
    dim3 buildBlock(10, this->maxFeature);
    ...
    buildForest<<<buildGrid, buildBlock, memory>>>(*d_x, *d_y, *d_mask, *d_branchClassCounts, *d_leafSamples, *d_tree, SAMPLES,
                                         featureSize, this->maxFeature, seed, SAMPLES+1, false);
...
ClassifierResult CudaCalibratedClassifierCV::fit(int *X, int *y, int *sample, int* foldBitMask, int* trainingFolds, int* validationFolds, int sampleSize, int featureSize, int seed) {
    ...
    allocateDeviceMemory(&d_x, X, &d_y, y, &d_mask, foldBitMask,&d_trainingFolds, trainingFolds, &d_validationFolds, validationFolds, &d_leafSamples, &d_branchClassCounts, &d_tree,
                         &d_predictions, &d_predictionProba, &d_validationProba, &d_sample, &d_weights, &d_bias, sample, sampleSize, featureSize, this->maxFeature,
    ...
    for (int i = 1; i < nFolds; i++) {
        ...
        // Build a Random Forest
        buildForest<<<buildGrid, buildBlock, totalSharedMemory>>>(d_x, d_y, d_trainingFolds + TRAINING_FOLD_SIZE * i, d_branchClassCounts, d_leafSamples, d_tree, sampleSize,
                                             featureSize, this->maxFeature, seed, validationFoldIndex, true);
    ...
    this->fitBaseRF(&d_tree, &d_leafSamples, &d_predictions, &d_predictionProba, &d_validationProba, &d_branchClassCounts, &d_mask, &d_x, &d_y, &d_sample, &h_predictions, seed, treeSize, maxLeafNodes, totalSharedMemory, featureSize);
